...
DEFAULT_RUNTIME_INVENTORY_PATH = DEFAULT_CAPABILITY_PACK_ROOT / "runtime-inventory.json"


def _silica_x_meta() -> dict[str, str]:
    return {
        "project": PROJECT_NAME,
        "version": VERSION,
        "theme": VERSION_THEME,
        "component": "intel_pack",
    }
...
_FEATURE_DESCRIPTIONS: dict[str, str] = {
    ...
    "signal_fusion_lane": "Unified connector lanes for integrating multi-source intelligence into silica_x workflows.",
...
def build_capability_gap_report(
    ...
    """Build comparative gap report across scanned source packs."""

    if not profiles:
        return {
            "source_packs": 0,
            "leaders": {},
            "recommendations": ["No capability source packs were discovered."],
        }
    ...
    for feature in _FEATURE_PATTERNS:
        ...
        leaders[feature] = {
            "score": top_score,
            "source_packs": [item.framework for item in ranked if item.feature_hits.get(feature, 0) == top_score],
        }
...
def render_capability_markdown(profiles: Sequence[FrameworkCapabilityProfile]) -> str:
    """Render a compact markdown summary for scanned source pack capabilities."""
    ...
    lines = [
        ...
        "",
        f"Source packs scanned: {len(profiles)}",
        "",
...
def write_capability_report(
    output_path: str | Path = "docs/silica_x-capability-scan.md",
    *,
    build_pack: bool = True,
) -> Path:
    """Scan source packs and write markdown capability report to disk."""
...
def build_runtime_inventory_snapshot(
    *,
    plugin_count: int,
    filter_count: int,
    platform_count: int,
    ...
    return {
        ...
        "inventory": {
            ...
            "error_counts": {
                "plugin_errors": int(plugin_error_count),
                "filter_errors": int(filter_error_count),
                "platform_error": int(bool(platform_error)),
...
def write_runtime_inventory_snapshot(
    snapshot: dict[str, object],
    *,
    output_path: str | Path = DEFAULT_RUNTIME_INVENTORY_PATH,
) -> Path:
    """Persist a runtime inventory snapshot to disk."""
...
def _build_plugin_capability_views(
    ...
    *,
    ...
    readme_lines = [
        "# Plugin Intel Views",
        "",
        "silica_x-native plugin recommendations by workflow.",
        "",
...
def build_capability_pack(
    ...
    *,
    ...
    baseline_payload = {
        ...
        "feature_count": len(_FEATURE_PATTERNS),
        "feature_presence_total": sum(feature_presence.values()),
